...
public class DbManager {
    ...
    public int generateBills(){
        int[] numRows;
        String sql_select = "SELECT phone_number,client_id, plan_cost, plan_minutes, plan_data_cap, plan_text_message FROM ((SELECT phone_number, client_id, plan_id FROM public.phone WHERE phone_number NOT IN (SELECT phone_number FROM public.active_bills) AND phone_date > current_timestamp(0)) AS verf_phone NATURAL JOIN public.plan) AS phone_to_plan";
        try {
            System.out.println("Consultando en la base de datos");
            PreparedStatement statement = connection.prepareStatement(sql_select);
            ResultSet resultSet = statement.executeQuery();
    ...
    public ArrayList<Bill> getAllBills(){
        ...
        try{
            ...
            while (resultSet.next()){
                array_bills.add(new Bill(resultSet.getDouble(4),resultSet.getDate(5),resultSet.getInt(6),resultSet.getInt(7),resultSet.getInt(8),
                        resultSet.getLong(3),new Client(resultSet.getInt(2),resultSet.getString(10),resultSet.getString(11),resultSet.getShort(16),resultSet.getString(12),
                        resultSet.getString(13),resultSet.getString(14),resultSet.getShort(15)),
                        new Plan(resultSet.getInt(1), resultSet.getString(17),
                                resultSet.getDouble(18),resultSet.getInt(19),resultSet.getInt(20),resultSet.getInt(21))));
            }
        } catch (SQLException e) {
            e.printStackTrace();
        } catch (Exception e){
